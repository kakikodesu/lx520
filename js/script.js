...
//随机文字烟花内容
const randomWords = ["柿子 ♥ 刘霞", "♥", "爱你永远", "浪漫永远", "幸福永远", "未来与你", "520"];
const wordDotsMap = {};
randomWords.forEach((word) => {
	wordDotsMap[word] = MyMath.literalLattice(word, 3, "黑体", "100px");
});
...
function randomShellName() {
	return Math.random() < 0.2 ? "Crysanthemum" : shellNames[(Math.random() * (shellNames.length - 1) + 1) | 0];
}
...
function getWordDots(word) {
	...
	var res = MyMath.literalLattice(word, 3, "黑体", fontSize + "px");
